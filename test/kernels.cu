...
unfused_lstm_update_c(int numElements, float *out, float *tmp1, float *tmp2,
                      ...
                      float *tmp7, float *tmp8, float *tmp9, float *tmp10,
                      float *tmp11, const float *c,
  ...
  // sigmoid(...) * tanh(...)
  pw_vecMul<<<gridDim, blockDim>>>(tmp11, tmp7, tmp10, numElements);

  // sigmoid(...) + sigmoid(...) * tanh(...)
  pw_vecAdd2<<<gridDim, blockDim>>>(out, tmp4, tmp11, numElements);
}
